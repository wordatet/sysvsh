...
register struct job *jp;
{
	...
	for (njp = &joblst; *njp && *njp != jp; njp = &(*njp)->j_nxtp)
		continue;
	if (*njp == NULL) return; /* Not found in list */
...
void freejobs()
{
	...
	if (jobdone) {
		register struct job *next;
		for (jp = joblst; jp; jp = next) {
			next = jp->j_nxtp;
			if (jp->j_flag & J_DONE)
				freejob(jp);
		}
