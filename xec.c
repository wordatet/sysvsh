	...
	if ((t = argt) && execbrk == 0) {
		...
		case TFORK:
		{
			...
			else if (com[0] != ENDARGS)
			{
				...
				rmtemp(0);
				clearjobs();
				if (comtype == BUILTIN) { /* If it's a builtin forced to fork */
					builtin(pos, argn, com, t); /* pos holds the builtin index */
					done(0); /* Exit child after builtin execution */
				} else {
					execa(com, pos);
				}
			}
